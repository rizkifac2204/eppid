...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,middlewares_PublicHandler__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .Z)().get(async (req, res)=>{
    const { id  } = req.query;
    const data = await libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"].from */ .Z.from("bawaslu").select("bawaslu.*", "admin.nama_admin").innerJoin("admin", function() {
        this.on("bawaslu.id", "=", "admin.bawaslu_id");
    }).where("bawaslu.id", id).first();
