...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,middlewares_PublicHandler__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .Z)().get(async (req, res)=>{
    const { id  } = req.query;
    const data = await (0,libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .Z)("provinsi").where("id", id).whereIn("id", function() {
        this.select("bawaslu_id").from("admin");
    }).first();
    const kabkota = await (0,libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .Z)("kabkota").where("provinsi_id", id).whereIn("id", function() {
        this.select("bawaslu_id").from("admin");
    }).orderBy("kabkota", "asc");
    const result = {
        result: data,
