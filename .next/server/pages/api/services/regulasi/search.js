...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,middlewares_PublicHandler__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .Z)().get(async (req, res)=>{
    ...
    const data = await libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"].select */ .Z.select(`regulasi.*`, `regulasi_kategori.kategori`).from("regulasi").innerJoin("regulasi_kategori", "regulasi.kategori_id", "regulasi_kategori.id").where((builder)=>{
        builder.where(true);
        if (kategori_id) builder.andWhere("regulasi.kategori_id", "=", kategori_id);
        if (kategori) builder.andWhere("regulasi_kategori.kategori", "like", `%${kategori}%`);
    ...
    }).orderBy("regulasi.kategori_id", "asc").orderBy("regulasi.nomor", "asc");
    if (data.length === 0) return res.status(200).json([]);
    const domain =  false ? 0 : process.env.HOST;
...
var __webpack_exports__ = (__webpack_exec__(1065));
