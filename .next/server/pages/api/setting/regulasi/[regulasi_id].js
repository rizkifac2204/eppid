...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,middlewares_Handler__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .Z)().get(async (req, res)=>{
    try {
        ...
        const result = await libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"].select */ .Z.select(`regulasi.*`, `regulasi_kategori.kategori`).from("regulasi").innerJoin("regulasi_kategori", "regulasi.kategori_id", "regulasi_kategori.id").where("regulasi.id", regulasi_id);
        res.json(result);
    } catch (error) {
    ...
    const { regulasi_id  } = req.query;
    try {
        const cek = await libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"].select */ .Z.select("berkas").from("regulasi").where("id", regulasi_id).first();
        const proses = await (0,libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .Z)("regulasi").where("id", regulasi_id).del();
        if (!proses) return res.status(400).json({
            message: "Gagal Hapus"
        });
        (0,services_UploadService__WEBPACK_IMPORTED_MODULE_2__/* .DeleteUpload */ ._0)("./public/regulasi", cek.berkas);
        res.json({
...
var __webpack_exports__ = __webpack_require__.X(0, [4383,1820,9733,8595], () => (__webpack_exec__(4467)));
