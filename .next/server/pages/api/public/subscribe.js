...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ((0,middlewares_PublicHandler__WEBPACK_IMPORTED_MODULE_1__/* ["default"] */ .Z)().post(async (req, res)=>{
    try {
        ...
        const cek = await libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"].select */ .Z.select("id").from("subscriber").where("email_subscriber", email).andWhere("bawaslu_id", bawaslu_id).first();
        if (cek) return res.json({
            message: "Terimakasih, Anda Sudah Menjadi Bagian Dari Kami"
        });
        const simpan = await (0,libs_db__WEBPACK_IMPORTED_MODULE_0__/* ["default"] */ .Z)("subscriber").insert({
            bawaslu_id,
            email_subscriber: email
        });
        if (!simpan) return res.status(404).json({
